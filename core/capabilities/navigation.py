...
class Navigation:
    ...
    def __init__(self, *, router: ToolRouter, memory: Any, slot_extractor: Any = None) -> None:
        self._router = router
        self._memory = memory
        # U7: an `async (kind, text) -> str | None` fallback used only when the
        # precise patterns miss. The regexes are the fast path and stay first;
        # this recovers unusual phrasings ("what's the best way over to
        # Chipotle") that nobody wrote a pattern for. None = regex only.
        self._slot_extractor = slot_extractor

    async def _slot(self, kind: str, text: str) -> str | None:
        if self._slot_extractor is None:
            return None
        try:
            return await self._slot_extractor(kind, text)
        except Exception:  # noqa: BLE001 — a slot miss must never break routing
            return None
    ...
    async def handle(self, text: str, *, current_location: dict[str, Any] | None = None) -> Reply | None:
        ...
        # Regex first (free, handles the common phrasings); the LLM slot
        # extractor only runs when it misses AND the message is clearly about
        # getting somewhere — see U7 in core/runtime.py::_llm_slot.
        local_destination = extract_destination_from_here(text) or await self._slot("destination", text)
